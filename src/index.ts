...
const autoTagUpdate = async (noteId: string, forceUpdate: boolean) => {
  ...
  const noteTags: string[] = (await getAll(joplin.data, ['notes', noteId, 'tags'], {
      fields: ['id'],
      page: 1
    })).map(t => t.id);
...
const debouncedTagUpdate = debounce((id: string) => autoTagUpdate(id, false), 5000, {trailing: true, leading: false})
...
joplin.plugins.register({
	onStart: async function() {
      ...
      console.debug("autoTag called for onNoteChange", event, ItemChangeEventType.Update);
